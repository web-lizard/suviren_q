...
def _create_shortcut_lnk(ico_path: Optional[Path]) -> None:
    """Create a .lnk on the desktop pointing to the PS1 script via ctypes (no win32com)."""
    try:
        desktop = Path(os.environ.get("USERPROFILE", "C:\\Users\\Default")) / "Desktop"
        lnk_path = desktop / SHORTCUT_LNK_NAME

        ps1_path = SHORTCUT_PS1.resolve()
        if not ps1_path.exists():
            warn(f"PS1 not found: {ps1_path}, cannot create shortcut")
            return
        ...
        $s.Arguments = '-NoExit -ExecutionPolicy Bypass -File "{ps1_path}"'
        $s.WorkingDirectory = '{PROJECT_ROOT}'
        $s.Description = 'Book Wunderwaffe 1.0 — CLI audiobook forge'
        """
...
def _create_ps1_script(venv_path: Optional[Path]) -> None:
    ...
    ps_content = f"""# Book Wunderwaffe 1.0 — CLI Launcher
# Generated by bookforge.py install-shortcut

Write-Host ""
Write-Host "  ╔══════════════════════════════════════════════╗"
Write-Host "  ║       Book Wunderwaffe 1.0                   ║"
Write-Host "  ║  La machine merveilleuse pour forger les     ║"
Write-Host "  ║                   livres                     ║"
Write-Host "  ╚══════════════════════════════════════════════╝"
Write-Host ""
...
$venvPath = '{venv_path or (PROJECT_ROOT / ".venv")}'
...
Write-Host "    python bookforge.py status     — Show project status"
Write-Host "    python bookforge.py chapters   — Extract chapters from RPP"
Write-Host "    python bookforge.py doctor     — Validate environment"
Write-Host "    python bookforge.py scan       — Scan data directory"
Write-Host "    python bookforge.py report     — Generate report"
Write-Host ""
Write-Host "  Render commands (use carefully):"
Write-Host "    python bookforge.py render-test --overwrite"
Write-Host "    python bookforge.py render-full"
Write-Host ""

# Run status by default
python bookforge.py status
"""
...
def _create_bat_script() -> None:
    """Create a .bat wrapper that launches the PS1 script."""
    bat_content = f"""@echo off
powershell.exe -NoExit -ExecutionPolicy Bypass -File "{SHORTCUT_PS1.resolve()}"
"""
