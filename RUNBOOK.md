...
- Во время синтеза карточка прогресса показывает процент, прошедшее время и
  текущий фрагмент. При озвучивании всей книги дополнительно видны текущая глава
  и количество уже обработанных глав.

### Передача книги в видео

- Достаточно открыть вкладку «Видео»: видеоверсия будет создана или обновлена
  автоматически. Нажатие `→ В видео` в плеере и перетаскивание озвучки на
  вкладку остаются быстрыми альтернативами.
- Активные озвучки автоматически собираются в мастер-аудио, а каждая готовая
  глава появляется отдельным клипом на аудиотаймлайне. Неозвученные главы
  добавляются после создания голоса и не ломают длительность текущего видео.
- После завершения новой озвучки существующая видеоверсия обновляется в фоне.
- Переключатель «Текст на экране» находится над предпросмотром. Он включает
  текущий читаемый фрагмент одновременно в плеере и итоговом MP4.
